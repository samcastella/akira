...
export default function MiActividadResumen() {
  ...
  // ✅ Evitar error de tipos: usamos foto y fallback
  const avatarSrc =
    (user as any)?.avatar_url ?? user?.foto ?? '/images/avatars/default.png';

  return (
    <div className="py-6 space-y-8">
      {/* Cabecera “CHECKS PARA HOY” + rueda con fuego */}
      <section>
        <div className="mb-2 flex items-baseline justify-between">
          <h2 className="text-xl font-semibold text-neutral-900">CHECKS PARA HOY</h2>
          <span className="text-sm text-neutral-500">
            {totalDone}/{totalGoal}
          </span>
        </div>
        <div className="relative">
          {/* pequeño fuego arriba a la izquierda de la rueda */}
          <span
            aria-hidden
      ...
      <section>
        ...
        <p className="text-sm text-neutral-600 mb-3">
          Descubre tus estadísticas de esta semana
        </p>
        <MiniWeeklyChart />
      </section>
      ...
      <section>
        ...
        <div className="[&_.ak-calendar-day]:flex [&_.ak-calendar-day]:items-center [&_.ak-calendar-day]:justify-center">
          {/* ↑ fuerza centrado de números si CalendarLite expone .ak-calendar-day por día */}
          <CalendarLite />
        </div>
      </section>
...
function ActiveProgramsList() {
  ...
  const entries = Object.entries(activeMap).filter(([, p]) => {
    const prog = p as any;
  ...
  return (
    <div className="space-y-3">
      {entries.map(([slug, p]) => {
        ...
        const currentDay =
          lp?.startedAt && totalDays ? dayFromStarted(lp.startedAt, totalDays) : 0;
...
/* ======= Mini gráfico semanal (vuelve el gráfico) ======= */
function MiniWeeklyChart() {
  // gráfico simple placeholder: las series reales las sacaremos de useTodayActivity pronto
  const width = 640,
    height = 220,
    padL = 28,
    padR = 16,
...
function AchievementsStrip() {
  // simple grid de insignias conocidas; cuando tengamos las reales, las leemos de DB
  const items = [
