...
export default function ActivosPage() {
  ...
  useEffect(() => {
    ...
    const slugs =
      store && typeof store === 'object' ? Object.keys(store as Record<string, unknown>) : [];
    setActiveSlugs(slugs);
  }, []);

  const existingPrograms = useMemo(() => EXISTING_PROGRAMS, []);
  const activePrograms = useMemo(
    () => existingPrograms.filter((p) => activeSlugs.includes(p.slug)),
    [existingPrograms, activeSlugs]
  );
  ...
  return (
    <div className="bg-white">
      ...
      <div className="px-4 pb-28 pt-4">
        <h1 className="text-3xl font-semibold tracking-tight mb-1">Programas activos</h1>
        <p className="text-sm text-neutral-500 mb-1">
          Los programas que estás siguiendo ahora mismo.
        </p>
        <p className="text-xs text-neutral-400 mb-4">
          {activePrograms.length} programas activos
        </p>

        <div className="divide-y divide-neutral-100">
          {activePrograms.length === 0 ? (
            <p className="text-sm text-neutral-500 py-4">
              Todavía no tienes ningún programa activo. Empieza uno desde la sección de
              programas o desde un reto comunitario.
            </p>
          ) : (
