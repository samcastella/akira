...
export default function GuardadosPage() {
  ...
  const savedPrograms = useMemo(
    () => EXISTING_PROGRAMS.filter((p) => saved.has(p.id)),
    [saved]
  );
  ...
  return (
    <div className="bg-white">
      ...
      <div className="px-4 pb-28 pt-4">
        <h1 className="text-3xl font-semibold tracking-tight mb-1">Guardado</h1>
        <p className="text-sm text-neutral-500 mb-1">
          Tus programas favoritos, listos para empezar cuando quieras.
        </p>
        <p className="text-xs text-neutral-400 mb-4">
          {savedPrograms.length} programas guardados · {activeCount} activos ahora mismo
        </p>

        <div className="divide-y divide-neutral-100">
          {savedPrograms.length === 0 ? (
            <p className="text-sm text-neutral-500 py-4">
              Todavía no tienes programas guardados. Toca el icono de marcador en cualquier
              programa para añadirlo aquí.
            </p>
          ) : (
            savedPrograms.map((p) => (
              <ProgramCard
