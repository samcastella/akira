...
/* ===== Programas (lectura 21 días, versión revisada) ===== */
...
function stopProgram(key: string) {
  const store = loadStore();
  delete store[key];
  saveStore(store);
}
...
function MiZona() {
  ...
  // Forzar rerender cuando cambiamos localStorage desde esta página
        ...
        {activeKeys.map((key) => {
          ...
          return (
            <div key={key} className="overflow-hidden rounded-2xl border" style={{ borderColor: COLORS.line }}>
              {/* Fila 1: nombre del programa (blanca, sin check) */}
              <div className="bg-white px-4 py-3">
                <div className="text-sm text-black/60">Programa</div>
                <div className="text-base font-semibold">{p.name}</div>
              </div>

              {/* Fila 2: tareas del día (gris claro) + check */}
