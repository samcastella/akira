...
/* ===== Layout helpers ===== */
const NAV_HEIGHT = 84; // alto barra inferior (~+30%)
...
function HabitCard({
  ...
  return (
    <div
      ...
      onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && onStart(data.key)}
      className="relative overflow-hidden"
    >
      {/* Contenedor 4:5 por padding-bottom (evita bug de aspect-ratio en Safari) */}
      <div
        className="relative w-full"
        style={{
          height: 0,
          paddingBottom: '125%', // 4:5 → 5/4 = 1.25 → 125%
          backgroundColor: '#111', // fondo mientras carga
        }}
      >
        <Image
          src={data.image}
      ...
      <div className="pointer-events-none absolute inset-0 flex flex-col justify-end p-5">
        ...
        {/* Si quieres CTA interno, quita pointer-events-none y añade botón aquí */}
      </div>
    </div>
  );
}
    ...
    return (
      <div className="flex min-h-screen items-center justify-center" style={{ background: COLORS.accent }}>
        <div
          ...
          style={{ height: 0, paddingBottom: '177.78%' /* 9:16 splash vertical opcional */ }}
        >
              ...
              {FEATURED_HABITS.map((h) => (
                <div key={h.key}>
                  <HabitCard
                    data={h}
                    onStart={(key) => {
                      // Aquí luego abriremos el detalle del hábito
                      // De momento solo mostramos un aviso:
                      alert(`Abrir programa: ${key}`);
                    }}
                  />
                </div>
              ))}
