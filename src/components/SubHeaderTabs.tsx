// src/components/SubHeaderTabs.tsx
'use client';
...
export default function SubHeaderTabs({
  ...
  // Normaliza path (sin barra final) para evitar falsos positivos
  const pathname = (usePathname() ?? '').replace(/\/+$/, '') || '/';
  const hCls = size === 'compact' ? 'h-10' : 'h-11';
  const txt = size === 'compact' ? 'text-[13px]' : 'text-sm';

  // Normaliza hrefs
  const norm = tabs.map(t => ({ ...t, nhref: (t.href.replace(/\/+$/, '') || '/') }));

  // Candidatos activos:
  // - exact?: true  -> solo coincide exacto
  // - exact?: false -> exacto o prefijo de subruta
  const candidates = norm.filter(t => {
    if (t.exact) return pathname === t.nhref;
    return pathname === t.nhref || pathname.startsWith(t.nhref + '/');
  });

  // Escoge un ÚNICO activo por "longest prefix match"
  const activeHref =
    candidates.sort((a, b) => b.nhref.length - a.nhref.length)[0]?.nhref ?? '__none__';

  return (
    <div className="bg-white border-b" role="navigation" aria-label={ariaLabel}>
