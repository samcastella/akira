...
import { getProgramMeta } from "@/data/programs"; // ← nuevo registro

type Props = {
  /** Si lo pasas, lo usa directamente. Si no, intenta resolverlo desde Supabase auth */
  userId?: string;
  /** Programa a mostrar (slug). Por defecto, detox para este ejemplo. */
  programSlug?: string;
...
export default function ProgramActiveCard({
  userId: userIdProp,
  programSlug = "detox-tecnologico-30",
  onReset,
}: Props) {
  ...
  const [totalDays, setTotalDays] = useState<number>(meta?.days ?? 30);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!isActive) {
          // no mostrar nada: el padre decide el estado vacío
          setTasks([]);
          setDaysCompleted(0);
          setTotalDays(meta?.days ?? 30);
          setCurrentDay(1);
          setLoading(false);
          return;
        }
  ...
  }, [supabase, userId, programSlug, meta?.days]);
  ...
  async function handleToggle(taskId: string, next: boolean) {
    ...
    try {
      ...
      if (advanced && nextDay) {
        // Refrescar progreso + tareas del nuevo día
  ...
  // Si no hay usuario o no hay programa activo, no mostramos la card
  if (!userId || !hasActive) return null;
  ...
  return (
    <div className="card">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h2 className="text-lg font-extrabold leading-tight">
            {meta?.titleShort ?? "Programa"}
          </h2>
          <p className="mt-1 text-sm muted">
            Vas por el <strong>Día {currentDay}</strong> · {percent}% completado
          </p>
        </div>
      ...
      {/* Checklist del día (si el día no trae tareas, mostramos mensaje) */}
      <div className="mt-6">
        <div className="mb-2 flex items-center gap-2">
          <Info className="h-4 w-4 muted" />
          <span className="text-sm muted">
            Marca cada mini-hábito en <strong>Mi Zona</strong>. Toca <strong>+</strong> para ver el detalle.
          </span>
        </div>
